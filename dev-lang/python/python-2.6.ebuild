...
# ToDo(msb): Undo hack below which removes python-update PDEPEND
# python-update is not required currently for chrome-os
#msb: PDEPEND="app-admin/python-updater"
...
cp -v /usr/lib/python${PYVER}/lib-dynload/unicodedata.so Modules/.
cp -v /usr/lib/python${PYVER}/lib-dynload/time.so Modules/.
cp -v /usr/lib/python${PYVER}/lib-dynload/math.so Modules/.
sed -i \
